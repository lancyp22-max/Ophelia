...
- The state file records timestamps only. A send attempt reserves its timestamp
  before the provider call so a crash after provider acceptance cannot immediately
  bypass cooldown protection.
- Console output contains a one-way, truncated destination fingerprint and the
  provider message identifier, never the destination or message body.
- Dry-run is the default. A provider call requires the explicit `--send` flag.
- Locking is implemented with Python standard-library primitives on both POSIX
  and Windows; no additional dependency is required.
